...
{
    public partial class PurchaseRecord :TemplateForm
    {
        ...
        private void FilterByDate()
        {
            using (SqlConnection con = new SqlConnection(General.General.ConnectionString))
            {
                string query = "SELECT i.PurchaseId,C.Name,i.Date,i.Total,i.Discount,i.Net,i.Cash,i.Balance,i.[User] FROM tblPurchase i INNER JOIN tblSupplier C ON i.SupplierId=C.SupplierId where i.Date >= @fromdate and i.Date <= @toDate";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
        ...
        private void FilterByNameWithoutDate()
        {
            ...
            using (SqlConnection con = new SqlConnection(General.General.ConnectionString))
            {
                string query = "SELECT i.PurchaseId,C.Name,i.Date,i.Total,i.Discount,i.Net,i.Cash,i.Balance,i.[User] FROM tblPurchase i INNER JOIN tblSupplier C ON i.SupplierId=C.SupplierId where i.SupplierId = @Id";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.CommandType = CommandType.Text;
        ...
        private void FilterByNameDate()
        {
            using (SqlConnection con = new SqlConnection(General.General.ConnectionString))
            {
                string query = "SELECT i.PurchaseId,C.Name,i.Date,i.Total,i.Discount,i.Net,i.Cash,i.Balance,i.[User] FROM tblPurchase i INNER JOIN tblSupplier C ON i.SupplierId=C.SupplierId where i.Date >= @fromdate and i.Date <= @toDate and i.SupplierId = @Id";
                using (SqlCommand cmd = new SqlCommand(query, con))
                {
                    cmd.CommandType = CommandType.Text;
                    cmd.Parameters.AddWithValue("@fromDate", FromDate.Value.Date);
        ...
        private void PopulateGrid()
        {
            ...
            string query = "SELECT i.PurchaseId,C.Name,i.Date,i.Total,i.Discount,i.Net,i.Cash,i.Balance,i.[User] FROM tblPurchase i INNER JOIN tblSupplier C ON i.SupplierId=C.SupplierId";
            using (SqlConnection con=new SqlConnection(General.General.ConnectionString))
            {
                using(SqlCommand cmd=new SqlCommand(query, con))
                {
